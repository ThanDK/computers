...
function ComponentsPage() {
  ...
  useEffect(() => {
    ...
    if (pagination.pageIndex > 0)
      newSearchParams.set('page', pagination.pageIndex);
  ...
  useEffect(() => {
    const handleClickAway = (e) => {
      if (
        editingRowId &&
        stockEditWrapperRef.current &&
        !stockEditWrapperRef.current.contains(e.target)
      ) {
  ...
  const handleStockUpdate = useCallback(
    async (row, newStockValueStr) => {
      ...
      try {
        const updated = await updateComponentStock(
          component.id,
          quantityChange,
          token
        );
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        header: 'Stock',
        meta: { cellClassName: 'text-center-cell', width: '145px' },
        cell: ({ row }) => {
          const component = row.original;
              ...
              <div ref={stockEditWrapperRef}>
                <InputGroup size="sm" style={{ width: '120px' }}>
                  <Form.Control
                    ...
                    onKeyDown={(e) => {
                      if (e.key === 'Enter')
                        handleStockUpdate(row, e.target.value);
                    }}
                  />
                  <Button
                    variant="outline-success"
                    onClick={() =>
                      handleStockUpdate(row, stockValueInputRef.current.value)
                    }
                  >
                    <BsCheck />
                  </Button>
        ...
        cell: ({ row }) => (
          <div className="d-flex gap-2 justify-content-center">
            <Button
              ...
              onClick={() =>
                navigate(`/edit-component/${row.original.id}`, {
                  state: { from: location },
                })
              }
            >
